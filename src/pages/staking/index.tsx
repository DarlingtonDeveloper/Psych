...
import { StakingV3 } from "../../v3/Staking"
...
const StakingHomepage = () => {
  ...
  useEffect(() => {
    router.prefetch(PAGE.STAKING_DASHBOARD)
  }, [router])

  useEffect(() => {
    if (globalConnectStatus.connected) {
      router.push(PAGE.STAKING_DASHBOARD)
    }
      ...
      <MaxContainer css={{ minHeight: "calc(100vh - 16rem)" }}>
        <Content>
          <WalletContainer>
            ...
            >
              Enter the Mycelia to unlock rewards and gain access to new items
              as you complete the cycles.
              <br />
              <br />
              <strong>We are connected.</strong>
            </Section>
            <Section
              css={{ maxWidth: "15rem", marginLeft: "$lg" }}
              initial={{ scale: 0.9, opacity: 0 }}
              animate={{ scale: 1, opacity: 1 }}
