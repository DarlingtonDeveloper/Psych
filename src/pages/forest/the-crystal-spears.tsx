...
const CrystalSpears = () => {
  ...
  const { data, isLoading } = useQuery(
    ...
    },
    {
      ...
      enabled: sessionData.progress > 9 && sessionData.progress < 11,
  ...
  useEffect(() => {
    const isReadyForNextStep = sessionData.progress > 10
    if (isReadyForNextStep) {
      router.push(PAGE.FOREST_ZERO_HOUR)
    }
      ...
      onSuccess: () => {
        setSessionData((data) => ({
          ...
          progress: +Object.keys(pages).find(
            (key) => pages[+key] === "/forest/the-zero-hour"
          )!,
        }))
        setRiddleAnswer("")
      },
  ...
  useEffect(() => {
    router.prefetch("/forest/the-zero-hour")
  }, [router])
    ...
    return (
      <EnterContainer>
        <MetaHead
          ...
          link="/forest/the-crystal-spears"
        />
        <MaxContainer>
          <ContentContainer>
