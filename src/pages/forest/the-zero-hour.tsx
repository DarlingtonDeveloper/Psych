...
const ZeroHour = () => {
  ...
  const { data, isLoading } = useQuery(
    ...
    },
    {
      ...
      enabled: sessionData.progress > 10 && sessionData.progress < 12,
  ...
  useEffect(() => {
    const isReadyForNextStep = sessionData.progress > 11
    if (isReadyForNextStep) {
      router.push(PAGE.FOREST_INEXPLICABLE_CALM)
    }
      ...
      onSuccess: () => {
        setSessionData((data) => ({
          ...
          progress: +Object.keys(pages).find(
            (key) => pages[+key] === "/forest/the-inexplicable-calm"
          )!,
        }))
        setRiddleAnswer("")
      },
  ...
  useEffect(() => {
    router.prefetch("/forest/the-inexplicable-calm")
  }, [router])
