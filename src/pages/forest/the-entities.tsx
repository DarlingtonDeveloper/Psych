...
const Entities = () => {
  ...
  const { data, isLoading } = useQuery(
    ...
    },
    {
      ...
      enabled: sessionData.progress > 12 && sessionData.progress < 14,
  ...
  useEffect(() => {
    const isReadyForNextStep = sessionData.progress > 13
    if (isReadyForNextStep) {
      router.push("/forest/the-machines")
    }
      ...
      onSuccess: () => {
        setSessionData((data) => ({
          ...
          progress: +Object.keys(pages).find(
            (key) => pages[+key] === "/forest/the-machines"
          )!,
        }))
        setRiddleAnswer("")
      },
  ...
  useEffect(() => {
    router.prefetch("/forest/the-machines")
  }, [router])
