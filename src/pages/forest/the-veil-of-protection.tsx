...
const G4 = () => {
  ...
  const { mutate } = useMutation(
    async () => {
      const response = await axios.post(
        "/.netlify/functions/update-veil-progress",
        undefined,
        {
          headers: {
        ...
        setTimeout(() => {
          setSessionData((data) => ({
            ...
            progress: +Object.keys(pages).find(
              (key) => pages[+key] === "/forest/the-entities"
            )!,
          }))
          router.push("/forest/the-entities")
        }, 500)
      },
      onError: () => {
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      if (document) {
        const iframeDoc = (
          document.getElementById("the-veil-of-protection-game") as any
        )?.contentWindow
        if (iframeDoc) {
          const hasRun =
          ...
          if (!hasRun && hasRunRef.current && score >= 200) {
            mutate()
          }
  ...
  return (
    <G2Container>
      <MetaHead
        title="Psychedelics Anonymous - The Forest"
        description="This is forest page"
        link="/forest/the-veil-of-protection"
      />
      <MaxContainer>
        <ContentContainer>
          <iframe
            id="the-veil-of-protection-game"
            style={{
              maxWidth: "1440px",
              margin: "0 auto",
              width: "100%",
            ...
            src="/protect-circle/HTML5/index.html"
          />
        </ContentContainer>
      </MaxContainer>
    </G2Container>
