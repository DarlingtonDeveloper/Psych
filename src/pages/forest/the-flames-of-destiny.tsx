...
const G3 = () => {
  ...
  const { mutate } = useMutation(
    async () => {
      const response = await axios.post(
        "/.netlify/functions/update-flames-progress",
        undefined,
        {
          headers: {
        ...
        setTimeout(() => {
          setSessionData((data) => ({
            ...
            progress: +Object.keys(pages).find(
              (key) => pages[+key] === PAGE.FOREST_CRYSTAL_SPEARS
            )!,
          }))
          router.push(PAGE.FOREST_CRYSTAL_SPEARS)
        }, 500)
      },
      onError: () => {
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      if (document) {
        const iframeDoc = (
          document.getElementById("the-flames-of-destiny-game") as any
        )?.contentWindow
        if (iframeDoc) {
          const hasRun =
      ...
      <MaxContainer>
        <ContentContainer>
          <iframe
            ...
            src="/escape-sideways/HTML5/index.html"
          />
        </ContentContainer>
      </MaxContainer>
    </G2Container>
