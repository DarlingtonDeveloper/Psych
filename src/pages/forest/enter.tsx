...
const Riddle = () => {
  ...
  const { data, isLoading } = useQuery(
    ...
    },
    {
      ...
      enabled: sessionData.progress < 4 && sessionData?.progress > 0,
  ...
  useEffect(() => {
    const isReadyForAscension = sessionData.progress > 3
    if (isReadyForAscension) {
      router.push(PAGE.FOREST_ASCENSION)
    }
  ...
  useEffect(() => {
    router.prefetch("/forest/ascension")
  }, [router])

  const enterButtonText =
    sessionData?.progress === 1 ? "Enter the forest" : "Next"
        ...
        <ContentContainer>
          ...
          {isLoading ? (
            ...
            <>
              ...
              <EnterBtn>
                <Button
                  disabled={(data?.answerLength || 0) !== riddleAnswer.length}
                  onClick={async () => {
                    if (data) {
                      try {
