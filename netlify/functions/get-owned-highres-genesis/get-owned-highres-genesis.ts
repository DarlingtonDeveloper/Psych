...
const getOwnedGenesis = async (ownerAddress: string): Promise<number[]> => {
  ...
  return response.data.ownedNfts.map((nft) => +nft.id.tokenId)
}
...
export const handler: Handler = async (event) => {
  ...
  try {
    ...
    const genesisIds = Array.from(
      new Set([...ownedGenesisIds, ...stakedGenesisIds.map((tid) => +tid)])
    )
    ...
    return {
      ...
      body: JSON.stringify(
        genesisIds.map((tid) => ({
          tokenId: tid,
          link: `https://genesis.psychedelicsanonymous.com/${parseInt(
            tid
          )}-Enhanced.png`,
          preview: `${GenesisCDNUrl}${tid}.jpg`,
        }))
      ),
    }
  } catch (error) {
